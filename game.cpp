...
static void EnumExistingSaveGames(Menu *menu)
{
	...
	for (i = 0; i < Max; i++)
	{
		...
		snprintf(gamename, sizeof (gamename), "Partida %d - [%s]", i + 1, str);
...
// If multiplayer is disabled, leave out this code to make it harder to hack back in
	...
	AndroidSetScreenMode(TOUCH_SCREEN_MENU);
				...
				case ACTION_LOAD_GAME:
					...
					if (PickFile("ELEGIR PARTIDA", EnumExistingSaveGames, szFileSaved, sizeof(szFileSaved)) == 0)
                    {
						int slotNum = 0;
						if (sscanf(szFileSaved, "Partida %d", &slotNum) == 1 && slotNum >= 1)
						{
							snprintf(szFileSaved, sizeof(szFileSaved), "%s/%d.gme", SAVEGAME_DIR, slotNum - 1);
						}
						else
						{
