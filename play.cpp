		...
		void DoMenu(
			...
			switch (ms_menuaction)
				...
				case MenuActionSaveGame:
					...
					#if 1 //PLATFORM_UNIX
					if (PickFile("ELEGIR PARTIDA", EnumSaveGamesSlots, szFile, sizeof(szFile)) == 0)
                    {
						int slotNum = 0;
						if (sscanf(szFile, "Partida %d", &slotNum) == 1 && slotNum >= 1)
						{
							snprintf(szFile, sizeof(szFile), "%s/%d.gme", SAVEGAME_DIR, slotNum - 1);
						}
						else
						{
